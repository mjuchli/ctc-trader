...
class Trade:
    def __init__(self, orderType, cty, price):
...
class ExecutorMock(Executor):
    ...
    def buy(self, price, pc = 1.0):
        ...
        t = Trade(OrderType.BUY, cty, price)
        self.report(t)
        return t

    def sell(self, price, pc = 1.0):
        balance = self.getAvailableCrypto()
        cty = pc * balance * price * (1-self.fee)
        self.balance['balance'] = balance - pc * balance
        self.balance['available'] = self.balance['balance']
        self.balance['fiat'] = self.balance['fiat'] + cty
        t = Trade(OrderType.SELL, cty, price)
        self.report(t)
        return t
